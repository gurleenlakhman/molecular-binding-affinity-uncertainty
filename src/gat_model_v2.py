"""GAT v2: train on 7-feature PyG molecular graphs, predict pChEMBL values."""
...
NODE_IN_DIM = 7       # [atomic_num, degree, is_aromatic, formal_charge, num_explicit_hs, num_implicit_hs, is_in_ring]
HIDDEN_DIM = 128
NUM_HEADS = 4
DROPOUT = 0.2
LR = 1e-3
...
def load_data() -> dict[str, list]:
    print("Loading molecular graphs (v2: 7 atom features)...")
    datasets = {}
    for split in ("train", "val", "test"):
        graphs = torch.load(DATA_DIR / f"graphs_v2_{split}.pt", weights_only=False)
...
def save_scatter(preds: np.ndarray, targets: np.ndarray, rmse: float, r2: float):
    ...
    ax.set_title(f"GAT v2 — test set\nRMSE={rmse:.3f}  R²={r2:.3f}")
    ax.legend()
    plt.tight_layout()
    out = RESULTS_DIR / "gat_scatter_v2.png"
...
def main():
    ...
    model_path = RESULTS_DIR / "gat_model_v2.pt"
