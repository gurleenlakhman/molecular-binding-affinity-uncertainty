"""Featurize SMILES into Morgan fingerprints and PyG molecular graphs."""
...
def main():
    ...
    # Morgan fingerprints
    print("\nBuilding Morgan fingerprints (radius=2, nBits=2048)...")
    morgan_arrays = build_morgan_arrays(dfs)
    out_path = DATA_DIR / "morgan_fps.npz"
    np.savez_compressed(out_path, **morgan_arrays)
    print(f"Saved fingerprints -> {out_path}")

    # PyG graphs
    print("\nBuilding PyG molecular graphs...")
    for split in splits:
        graphs = build_graphs(dfs[split], split)
        out_path = DATA_DIR / f"graphs_{split}.pt"
